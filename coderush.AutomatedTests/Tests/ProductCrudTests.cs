...
[TestClass]
public sealed class ProductCrudTests : SeleniumTestBase
{
    [TestInitialize]
    ...
    [TestMethod]
    public void ProductGrid_Loads_DisplaysSeededData()
    {
        var rows = Driver.FindElements(By.CssSelector(".e-grid .e-row"));
        Assert.IsTrue(rows.Count > 0,
            "Grid should display seeded product rows.");
    }

    [TestMethod]
    public void ProductGrid_HasColumnHeaders()
    {
    ...
    [TestMethod]
    public void ProductGrid_Add_OpensEditRow()
    {
        ...
        Assert.IsTrue(editableInputs.Count > 0,
            "Clicking Add should open an editable product form.");
    }
}